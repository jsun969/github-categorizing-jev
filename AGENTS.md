...
- Approved radius theme: S uses `--radius: 0.25rem`; form radius M uses `--field-radius: 0.5rem` (12px buttons/cards/dialogs and 8px fields at the default 16px root font size). Keep other HeroUI theme defaults.
...
- Use native HeroUI button variants for navigation links, `Meter` for probabilities, `ProgressBar` for job progress, and `Tooltip` for contextual hints.
...
| Compact Workspace toolbar, HeroUI table, probability meters, selection, and jobs | `src/routes/index.tsx`, `src/components/workspace/`                                                       |
